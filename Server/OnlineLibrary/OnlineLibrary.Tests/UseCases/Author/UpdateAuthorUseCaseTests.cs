...
namespace OnlineLibrary.Tests.UseCases.Author
{
    public class UpdateAuthorUseCaseTests
    {
        ...
        public async Task ExecuteAsync_ShouldUpdateAuthor()
        {
            ...
            _mockMapper.Setup(m => m.Map(authorDTO, existingAuthor)).Returns(updatedAuthor);

            // Act
            await _useCase.ExecuteAsync(authorId, authorDTO, CancellationToken.None);

            // Assert
            mockRepo.Verify(r => r.Update(existingAuthor), Times.Once);
        ...
        public async Task ExecuteAsync_ShouldThrowEntityNotFoundException_WhenAuthorNotFound()
        {
            ...
            // Act & Assert
            await Assert.ThrowsAsync<EntityNotFoundException>(() => _useCase.ExecuteAsync(authorId, authorDTO, CancellationToken.None));
        }
    }
}
